...
public class EmpDao {
	...
	public boolean insertEmp(Employee emp) {
		String query = "insert into emp_details(id, name, password, leaves, duty, salary) values(?, ?, ?, ?, ?, ?)";
...
//			con.close();
	...
	public ArrayList<Employee> selectEmpInfor() {
		...
		try {
			...
			while (rs.next()) {
				...
				empList.add(new Employee(id, name, password, leaves, duty,req_leave, req_duty, salary));
	...
	public int getReqDuty(int emp_id) {
        ...
        public int getSalary(int emp_id){
            String query = "select salary from emp_details where id = ?";
            int salary = 0;
            try {
			PreparedStatement ps = this.con.prepareStatement(query);
			ps.setInt(1, emp_id);
        ...
        public int changeSalary(int emp_id) {
            int salary = 0;    
            if(getDuty(emp_id)>0){
                    manager.EmpManager empManager = new EmpManager();
                    int duty = empManager.getDuty(emp_id);
                    salary = duty * 10000;
                }
		String query = "update emp_details set salary = ? where id = ?";
